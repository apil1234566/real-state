...
class FriendController extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->role == 1) {
            $owner_room_ids = Owner::where('user_id', auth()->id())->first()->properties()->pluck('id');
            $all_applicants = Applicant::distinct()->whereIn('property_id', $owner_room_ids)->pluck('user_id');
            ...
            $applicants_room_ids = Applicant::distinct()->where('user_id', auth()->id())->pluck('property_id');
            $user_rooms = Property::distinct()->whereIn('id', $applicants_room_ids)->pluck('user_id');
